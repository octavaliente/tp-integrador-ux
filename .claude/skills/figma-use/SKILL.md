...
**If the task involves illustration, artistic, or expressive visual work — posters, badges, decorative graphics, illustrated scenes, painterly designs, or any task that calls for hand-crafted visuals rather than clean product UI** — load [figma-use-draw](../figma-use-draw/SKILL.md) BEFORE writing any code. It covers brushes (stretch/scatter), texture and noise effects, dynamic and variable-width strokes, pattern paints, text on a path, gradient fills, and TransformGroups (radial repeaters) — features that ARE the illustration, not optional polish. When sharing a Figma URL back to the user from a draw flow, append `m=draw` so the file opens in Draw mode.
